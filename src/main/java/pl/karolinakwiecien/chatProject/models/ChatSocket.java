package pl.karolinakwiecien.chatProject.models;
...
public class ChatSocket extends TextWebSocketHandler implements WebSocketConfigurer {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        if(isUserBanned(userModel)){
            userModel.sendMessage("server:You are banned! ");
            userModel.sendMessage("server: Still:: " + (userModel.getBanTime().until(LocalTime.now(), ChronoUnit.SECONDS)) + "s.");
            return;
        }
        ...
        if(userModel.getUsername() == null){
            if(isNickBusy(messageString)){
                userModel.sendMessage("server:This nickname is busy, try another one");
                return;
            }
            userModel.setUsername(messageString);
            userModel.sendMessage("server:I set your nickname!");
            return;
        }
        ...
        sendMessageToAll("log:" + userModel.getUsername() + "<" + getTime() + ">: " + message.getPayload());
    }

    private void kickCommand(UserModel userModel, String messageString) throws IOException {
        String data[] = messageString.split(" ");
        if(data.length < 2){
            userModel.sendMessage("Too few characters.");
            return;
        }
        Optional<UserModel> userToKick = findUserByUsername(data[1]);
        if(!userToKick.isPresent()){
            userModel.sendMessage("This user doesn't exist!");
            return;
        }

        userToKick.get().sendMessage("You were kick out!");
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        userModel.sendMessage("server:Welcome on my chat!");
        userModel.sendMessage("server:Your first message will be your nickname:");
